...
<script type="text/javascript">
	...
	function roleTypeBar(ec){
        ...
        $.ajax({
            ...
            url : "${pageContext.request.contextPath}/workbench/employeeTypeWorkbench.action",//请求发送到TestServlet处
            data : {},
            dataType : "json", //返回数据形式为json
            success : function(employeeCharts) {
            	//var roleType = eval('(' + employeeCharts + ')');
            	//alert(employeeCharts);
            	var resultLength = getJsonLength(employeeCharts);
            	//alert(resultLength);
            	//请求成功时执行该函数内容，result即为服务器返回的json对象 keyword keywordCount
                if (employeeCharts != null && employeeCharts !="") {
                   //挨个取出类别并填入类别数组 //x坐标的值
                   for(var i=0; i<resultLength; i++){
                	   //alert(result.list[i].bookName);
                	   //console.info(result[i]);
                	   names.push(employeeCharts[i].jxRole.roleName);   
                    }
                   //alert(names);
                   //挨个取出销量并填入销量数组 //y坐标的值
                   for(var i=0; i<resultLength; i++){
                       nums.push(employeeCharts[i].roleCount);    
                   }
                   //alert(nums);
                   //隐藏加载动画
                   myChart.hideLoading(); 
	...
	function employeeSalary(ec){
        ...
        $.ajax({
            type : "POST",
            //async : true, //异步请求（同步请求将会锁住浏览器，用户其他操作必须等待请求完成才可以执行）
            url : "${pageContext.request.contextPath}/workbench/selectByEmployeeSalary.action",//请求发送到TestServlet处
            data : {},
            dataType : "json", //返回数据形式为json
            success : function(employeeSalaryCharts) {
            	//var employeeSalaryCharts = eval('(' + employeeSalaryCharts + ')');
            	//alert(employeeSalaryCharts);
            	var resultLength = getJsonLength(employeeSalaryCharts);
            	//alert(resultLength);
            	//请求成功时执行该函数内容，result即为服务器返回的json对象 keyword keywordCount
                if (employeeSalaryCharts != null && employeeSalaryCharts !="") {
                   //挨个取出类别并填入类别数组 //x坐标的值
                   for(var i=0; i<resultLength; i++){
                	   //alert(result.list[i].bookName);
                	   //console.info(result[i]);
                	   names.push(employeeSalaryCharts[i].employeeExtend);   
                    }
                   //alert(names);
                   //挨个取出销量并填入销量数组 //y坐标的值
                   for(var i=0; i<resultLength; i++){
                       nums.push(employeeSalaryCharts[i].salaryCount);    
                   }
                   //alert(nums);
                   //隐藏加载动画
                   myChart.hideLoading(); 
