...
<form action="saleOrder/jxcPurchaseorderStockSelect.action" class="form-inline" method="post">
		...
		<div class="row" style="padding: 15px;text-align: center;">
			<table class="table table-hover table-bordered table-striped table-condensed">
				...
				<tr class="info">
					...
					<td>
						<c:if test="${resultPurchaseorder.orderNumber > 0}">
							<%-- <a class="btn btn-success" 
								href="saleOrder/saleOrderSuccess.action?
								orderId=${resultPurchaseorder.orderId}
								&orderNumber=${resultPurchaseorder.orderNumber }
								&supplierName=${resultPurchaseorder.jxcSupplier.supplierName}
								&employeeName=${resultPurchaseorder.jxcEmployee.employeeName}
								&warehouseName=${resultPurchaseorder.jxcWarehouse.warehouseName}
								&goodsName=${resultPurchaseorder.jxcGoods.goodsName}
								
								">
								销售</a> --%>
								<a class="btn btn-success" 
								href="saleOrder/saleOrderSingle.action?
								orderId=${resultPurchaseorder.orderId}
								">
								销售</a>
						</c:if>
						<c:if test="${resultPurchaseorder.orderNumber <= 0}">
							<a class="btn btn-default" onclick="return false;" href="saleOrder/saleOrderSuccess.action?orderId=${resultPurchaseorder.orderId}&orderNumber=${resultPurchaseorder.orderNumber }">
								销售</a>
						</c:if>
						<!-- <button id="saleGoods" class="btn btn-success">
							销售
						</button> -->
					</td>
				</tr>
	  ...
	  <div class="row">
		...
		<div class="col-md-4">
			<nav aria-label="Page navigation">
			  <ul class="pagination">
				<!-- 首页点击功能saleOrder/jxcPurchaseorderStockSelect.action -->
				<li><a href="${basePath}saleOrder/jxcPurchaseorderStockSelect.action?pn=1">首页</a></li>
				<!-- 上一页点击功能 -->
				<c:if test="${pageInfo.hasPreviousPage }">
					 <li>
					  <a href="${basePath}saleOrder/jxcPurchaseorderStockSelect.action?pn=${pageInfo.pageNum-1}" aria-label="Previous">
				...
				<c:forEach items="${pageInfo.navigatepageNums }" var="page_Num">
					...
					<c:if test="${page_Num != pageInfo.pageNum }">
						<li>
							<a href="${basePath}saleOrder/jxcPurchaseorderStockSelect.action?pn=${ page_Num}">
								${ page_Num}
							</a>
						</li>
					</c:if>
				...
				<c:if test="${pageInfo.hasNextPage }">
					<li>
					  <a href="${basePath}saleOrder/jxcPurchaseorderStockSelect.action?pn=${pageInfo.pageNum+1}" aria-label="Next">
				...
				<!-- 末页点击功能 -->
				<li>
					<a href="${basePath}saleOrder/jxcPurchaseorderStockSelect.action?pn=${pageInfo.pages}">
						末页
					</a>
				</li>
			  </ul>
